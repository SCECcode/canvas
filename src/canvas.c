...
//fprintf(stderr,"coord %d %d %d\n", load_x_coord, load_y_coord, load_z_coord);
		...
		y_percent = fmod((lat_n - canvas_configuration->bottom_left_corner_lat), delta_lat)/
delta_lat;
...
//fprintf(stderr,"percent %lf %lf %lf\n", x_percent, y_percent, z_percent);
		if (load_z_coord == 0 && z_percent == 0) {
		  ...
		  if( canvas_configuration->interpolation) {
	       ...
	       data[i].vs = canvas_calculate_vs(data[i].vp);
	}
...
void canvas_read_properties(int x, int y, int z, canvas_properties_t *data) {
	...
	int location = z * (canvas_configuration->nx * canvas_configuration->ny) + (y * canvas_configuration->nx) + x;
